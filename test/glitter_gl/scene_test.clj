...
;; scene/plan is a plain (fn [state scene-fn] -> render-plan) — no reaction, no
;; deref. glitter's own state-atom watcher recomputes the whole view on every
;; state change, so "recomputes when state changes" here means "call plan
;; again with the new state," mirroring glitter's top-down re-render model
;; rather than glimmer-gl's per-cell dependency tracking.
