...
and renormalized) in `:smooth` mode. It then interleaves position and
...
but the three that matter for the vertex layout are: one `gl-buffer-data`
to copy the whole interleaved blob into a single VBO,
...
last: translation lives in the *last group of four* values, not
