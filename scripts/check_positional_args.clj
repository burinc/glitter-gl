...
;;
;; Adapted from glitter's scripts/check_positional_args.clj (itself
;; adapted from b12n-adk-clj's) — source-dirs pointed at glitter-gl's
;; src/glitter_gl layout, and parse-arg-vector filters out ^Type hint
;; tokens and the & rest-arg marker before counting (see that fn's
;; comment) — a fix glitter's own copy doesn't need, since its code
;; doesn't lean on type hints the way glitter-gl's ported geometry/matrix
;; layer does.
...
(def file-pattern
  ...
  sitting directly in source-dirs (glitter-gl's src/glitter_gl is flat —
  25 direct .clj children, no subdirs — so that pattern would find zero
...
;;
;; Matched by function name only, not file — a future same-named fn in a
;; different (non-verbatim-port) file would also be silently excepted.
;; Given the current 1:1 name/file correspondence below, accepted for now;
;; revisit if a name collision with a refactorable file ever shows up in
;; the check's output.
(def exceptions
  #{;; bezier.clj
    "cubic-point" "quadratic-point" "cubic-tangent" "sample-cubic" "sample-quadratic"
    ;; gl.clj
    "shader-status" "make-tf-program"
    ;; intersect.clj
    "ray-sphere" "ray-plane" "ray-triangle" "ray-aabb"
    "sphere-sphere?" "aabb-aabb?" "aabb-sphere?"
    ;; matrix.clj
    "invert" "transform-point" "translation" "scaling" "perspective" "ortho" "look-at"
    ;; mesh.clj
    "translate" "scale" "face-normal" "subdivide-tri" "->floats"
    ;; plane.clj
    "plane-from-points"
    ;; polygon.clj
    "tri-signed-area2" "point-in-tri2?" "ear-at"
    ;; primitives.clj
    "cuboid" "ortho-normal" "orient-tetra"
    ;; quaternion.clj
    "quat"
    ;; shader.clj
    "set-uniform!"
...
(defn parse-arg-vector
  ...
  (when arg-str
    (let [;; Remove comments
          ...
          ;; a positional parameter itself. Unfiltered, glitter-gl's
