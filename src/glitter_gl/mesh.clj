(ns glitter-gl.mesh
  ...
  A mesh is a sequence of faces; each face is a vector of `glitter-gl.vector`
  ...
  build primitives (see `glitter-gl.primitives`), `transform`/`merge`/`subdivide`
...
(defn transform
  "Push every vertex through a `glitter-gl.matrix` Matrix44 (thi.ng's
